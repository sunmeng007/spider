...
import com.iflytek.indexer.hadoop.indexer.WanDouJiaAppIndexer;
...
public class WanDouJiaHadoopIndexer extends OozieMain implements Tool {
  public static final Log LOG = LogFactory.getLog(WanDouJiaHadoopIndexer.class);
  ...
  public int index(List<Path> inputs, Path output, String coreName,
    ...
    WanDouJiaHadoopIndexer.setIndexerClass(job, clazz);
  ...
  public static void main(String[] args) throws Exception {
    int res = OozieToolRunner.run(new Configuration(), new WanDouJiaHadoopIndexer(),
        args);
    System.exit(res);
  }
  ...
  public int run(String[] args) throws Exception {
    Path segment = new Path(
        "/Users/sunmeng/spider/maple-spider-brand/wandoujiaappv2/segments");
    ...
    if (segments != null) {
      for (FileStatus s : segments) {
        Path appinfo = new Path(s.getPath(),
            AppWanDouJiaModel.class.getSimpleName());
